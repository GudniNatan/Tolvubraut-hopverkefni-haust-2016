...
class MazeScene(Scene):
    def __init__(self, level, difficulty=0):
        ...
        mazeBox.x = 28 * levelDrawSize - (
        (level + level % 2 - 1) * levelDrawSize)  # Change the first number to move maze
        ...
        if self.level % 2:
            self.exit = Block(pygame.Rect(mazeBox.left + levelDrawSize, mazeBox.top, levelDrawSize, levelDrawSize),
                              GREEN)
            self.entrance = Block(
                pygame.Rect(mazeBox.right - levelDrawSize * 2, mazeBox.bottom - levelDrawSize, levelDrawSize,
                            levelDrawSize), BLUE)
            ...
            self.exit = Block(
                pygame.Rect(mazeBox.right - levelDrawSize * 2, mazeBox.bottom - levelDrawSize, levelDrawSize,
                            levelDrawSize), GREEN)
            self.entrance = Block(pygame.Rect(mazeBox.left + levelDrawSize, mazeBox.top, levelDrawSize, levelDrawSize),
                                  BLUE)
        ...
        if level >= 5 and difficulty > 0:
            pygame.time.set_timer(stalkerEvent,
                                  5000 - (self.level * 50 if level < 50 else 250))  # Spawn stalker after 5 seconds
        ...
        if level >= 5:
            rect1 = pygame.Rect(random.randrange(1, len(self.maze) / 2, 2) * levelDrawSize + mazeBox.left,
                                random.randrange(1, len(self.maze[0]), 2) * levelDrawSize + mazeBox.top, drawSize,
                                drawSize)
            rect2 = pygame.Rect(
                random.randrange((len(self.maze) / 2) if (len(self.maze) / 2) % 2 else (len(self.maze) / 2) + 1,
                                 len(self.maze) - 1, 2) * levelDrawSize + mazeBox.left,
                random.randrange(1, len(self.maze[0]), 2) * levelDrawSize + mazeBox.top, drawSize, drawSize)
    ...
    def render(self, screen):
        ...
        for i in xrange(self.mazeBox.w / self.levelDrawSize):
            for j in xrange(self.mazeBox.h / self.levelDrawSize):
                screen.blit(self.floor_tile,
                            (i * self.levelDrawSize + self.mazeBox.left, j * self.levelDrawSize + self.mazeBox.top))
    ...
    def handle_events(self, events):
        for event in events:
            ...
            if event.type == pathfindingEvent and self.stalker:
                ...
                stalker_grid_pos = [self.stalker.collision_rect.centerx / self.levelDrawSize,
                                    self.stalker.collision_rect.centery / self.levelDrawSize]
                self.stalker.update_path(self.grid.grid, stalker_grid_pos, mouse_grid_pos, -1, 1000)
            if event.type == MOUSEMOTION:
                distance = (
                -(self.last_pos[0] - pygame.mouse.get_pos()[0]), -(self.last_pos[1] - pygame.mouse.get_pos()[1]))
                ...
                if dist2 > self.levelDrawSize ** 2 * 4 and (self.last_pos[1] - pygame.mouse.get_pos()[1]):
                    ...
                    pygame.mouse.set_pos(self.last_pos[0] + (distance[0] / factor),
                                         self.last_pos[1] + (distance[1] / factor))
                check_col = False
                ...
                if distance[0] ** 2 + distance[1] ** 2 > self.levelDrawSize ** 2:
                    coords1 = ((self.last_pos[0] + pygame.mouse.get_pos()[0] * 2) / 3,
                               (self.last_pos[1] + pygame.mouse.get_pos()[1] * 2) / 3)
                    coords2 = ((self.last_pos[0] + pygame.mouse.get_pos()[0]) / 2,
                               (self.last_pos[1] + pygame.mouse.get_pos()[1]) / 2)
                    coords3 = ((self.last_pos[0] * 2 + pygame.mouse.get_pos()[0]) / 3,
                               (self.last_pos[1] * 2 + pygame.mouse.get_pos()[1]) / 3)
                    for block in self.block_group:
                        if block.rect.collidepoint(coords1) or block.rect.collidepoint(
                                coords2) or block.rect.collidepoint(coords3):
                            if not self.godMode:
                                self.manager.go_to(GameOverScene(self.level))
                            else:
                                check_col = True
...
class TitleScene(Scene):
    ...
    def render(self, screen):
        ...
        screen.blit(self.text1, (200, 175))
